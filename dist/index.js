...
export function verifyRuntimeIntegrity() {
  if (!fs.existsSync(wasmPath)) throw new LbeError('RUNTIME_MISSING', 'LBE WASM runtime is missing');
  if (!fs.existsSync(lockPath)) throw new LbeError('RUNTIME_LOCK_MISSING', 'LBE WASM integrity lock is missing');
  ...
  const expectedBuffer = Buffer.from(lock.wasm_sha256, 'hex');
  const actualBuffer = Buffer.from(actual, 'hex');
  if (expectedBuffer.length !== actualBuffer.length || !crypto.timingSafeEqual(actualBuffer, expectedBuffer)) {
...
export function getRuntimeInfo() {
  return Object.freeze({
    ...
    runtime: 'wasm',
    offline: true,
    integrity: verifyRuntimeIntegrity()
  });
}
...
export function execute(input) {
  ...
  if (input.trim().length === 0) throw new LbeError('INPUT_EMPTY', 'execute input must not be empty');
  ...
  if (bytes.length + 1 > max) throw new LbeError('INPUT_TOO_LARGE', 'execute input exceeds WASM buffer');
